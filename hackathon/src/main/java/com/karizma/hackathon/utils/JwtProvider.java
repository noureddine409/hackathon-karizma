...
public class JwtProvider {
    ...
    public JwtToken generateToken(User user, GenericEnum.JwtTokenType tokenType) throws BusinessException {
        ...
        JWTCreator.Builder builder = JWT.create().withJWTId(UUID.randomUUID().toString()).withSubject(user.getId().toString()).withIssuedAt(creationDate);
    ...
    public JwtToken generateToken(User user, GenericEnum.JwtTokenType tokenType, String tokenId) throws BusinessException {
        ...
        JWTCreator.Builder builder = JWT.create().withJWTId(tokenId).withSubject(user.getId().toString()).withIssuedAt(creationDate);
    ...
    private JwtToken checkTokenType(User user, GenericEnum.JwtTokenType tokenType, Instant creationDate, JWTCreator.Builder builder) {
        ...
        try {
            switch (tokenType) {
                case ACCESS -> {
                    ...
                    expiryDate = LocalDateTime.now().plusMinutes(accessTokenExpirationInMins).atZone(ZoneId.systemDefault()).toInstant();

                    List<String> claims = user.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList());

                    builder.withExpiresAt(expiryDate).withClaim("roles", claims);
                }
                case REFRESH -> {
                    secret = refreshTokenSecret;
                    expiryDate = LocalDateTime.now().plusWeeks(refreshTokenExpirationInWeeks).atZone(ZoneId.systemDefault()).toInstant();

                    builder.withExpiresAt(expiryDate);
                }
                case RESET -> {
                    secret = resetTokenSecret;
                    expiryDate = LocalDateTime.now().plusMinutes(resetTokenExpirationInMins).atZone(ZoneId.systemDefault()).toInstant();
        ...
        return JwtToken.builder().token(token).createdAt(creationDate).expiresIn(expiryDate).build();
    }
